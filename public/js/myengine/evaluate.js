...
var alphaBetaSearchRoot = function (depth, game, isMaximisingPlayer) {
    ...
    for (var i = 0; i < moveLen; i++) {
        ...
        var value = alphaBetaSearch(depth - 1, game, alpha, beta, !isMaximisingPlayer);
        game.undo();
        if (value >= bestMove) {
            bestMove = value;
...
function getEngineMove() {
    ...
    var bestMove = alphaBetaSearchRoot(4, game, true);
...
function alphaBetaMinimax(depth, game) {

    ""
    " 
    Returns best score
    for the player associated with the given node.
    ...
    # initialize bestMove
    if node is root
    bestMove = operator of first child# check
